...
def preprocess_ml100k():
    ...
    print(f"年龄类型: {len(unique_ages)} 种，范围: {min(unique_ages)}-{max(unique_ages)}")
    ...
    # 计算特征维度
    user_feature_dim = len(unique_ages) + len(unique_genders) + len(unique_occupations)
    ...
    print(f"用户特征维度: {user_feature_dim} (年龄:{len(unique_ages)} + 性别:{len(unique_genders)} + 职业:{len(unique_occupations)})")
